...
test('parseWorkflowStatus reports activity failures, timeouts, and workflow failures', () => {
  ...
  expect(parsed.activities.map(activity => activity.id)).toEqual([11, 9])
  expect(parsed.activities[0]?.status).toBe('timed out')
  expect(parsed.activities[0]?.details).toBe('Timeout START_TO_CLOSE.')
  expect(parsed.activities[1]?.status).toBe('error')
  expect(parsed.activities[1]?.details).toBe('Message: download failed')
