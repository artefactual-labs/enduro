				...
				func() error {
					for {
						...
						default:
							...
							go func() {
								if err := collection.InitProcessingWorkflow(ctx, workflowClient, event, event.PipelineName, event.RetentionPeriod, event.StripTopLevelDir, event.Key, config.Validation); err != nil {
