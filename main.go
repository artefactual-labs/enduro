				...
				func() error {
					for {
						...
						default:
							...
							go func() {
								if err := collection.InitProcessingWorkflow(ctx, workflowClient, event, event.PipelineName, event.RetentionPeriod, event.StripTopLevelDir, config.Validation); err != nil {
