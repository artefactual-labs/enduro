				...
				func() error {
					for {
						...
						default:
							...
							go func() {
								if err := collection.InitProcessingWorkflow(ctx, workflowClient, event.WatcherName, event.PipelineName, event.TransferType, event.RetentionPeriod, event.StripTopLevelDir, event.Key, "", config.Validation); err != nil {
